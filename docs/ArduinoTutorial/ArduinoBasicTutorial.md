...
在编译代码或上传代码时，如果出现报错信息 “No such file or directory”，那说明缺少相应的库文件，如下图就是上传代码时因为缺少了库文件的报错。下面我们以"项目十八 旋转编码器模块计数"为例。
...
在这里，我们将为您介绍最简单的添加库的方法。我们是以添加“Encoder”库文件为例。

1\. 首先，依次点击左上角的 **“项目” --> “导入库” --> “添加.ZIP 库...”**

![Arduino_2.3.6-a45](./media/Arduino_2.3.6-a45.png)

2\. 导航到库文件所在的目录（在`2.1 代码和库文件下载`中下载的ZIP文件解压），打开文件`KE3003库文件`然后选择 `Encoder.zip`文件。

![Arduino_2.3.6-a46](./media/Arduino_2.3.6-a46.png)



3\. 安装完成后，你将收到一条通知(已从`Encoder.zip`存档成功安装库)，同时输出框会显示 “**Library installed**”，确认该库已成功添加到Arduino IDE中。下次需要使用此库时，你不需要重复安装过程。
...
# 2.2.7 使用Arduino IDE上传第一个程序

先将Keyes UNO R3开发板通过USB线连接到电脑。

![Arduino_2.3.6-a36](./media/Arduino_2.3.6-a36.png)

打开Arduino IDE, 选择对应的Arduino UNO开发板型号。

![Arduino_2.3.6-a37](./media/Arduino_2.3.6-a37.png)

选好开发板后再选择USB端口，在 “**工具**” 列表中选择 “**端口**” 然后选择 “COM30（Arduino Uno）”。
...
然后我们点击右上角的“串口监视器”图标![a42](./media/Arduino_2.3.6-a42.png)便能打开串口监视器，然后设置波特率为**9600**，就能看到串口打印字符串 “**Hello Keyes!**”
